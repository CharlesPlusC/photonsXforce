...
DT_SECONDS = 60.0
POSITION_NOISE_STD_M = 100
VELOCITY_NOISE_STD_MPS = 0.005
NUM_EPOCHS = 500
SEED = 7
...
PLOT_DIR = Path(str(PROJECT_ROOT) + "/plots/hp_inversion_noisy")
...
def plot_ground_tracks(lon_deg, lat_deg, alt_km, out_path):
...
def main():
    ...
    noise = rng_noise.normal(0.0, POSITION_NOISE_STD_M, size=(NUM_ORBITS, truth_states.shape[1], 3))
    observed_positions = truth_states[:, :, 0, :] + jnp.array(noise)
    vel_noise = rng_noise.normal(0.0, VELOCITY_NOISE_STD_MPS, size=(NUM_ORBITS, truth_states.shape[1], 3))
    observed_velocities = truth_states[:, :, 1, :] + jnp.array(vel_noise)
    ...
    def run_inversion(label, propagate_learned_vmapped):
        ...
        def compute_loss(params):
            ...
            pos_err = jnp.mean(jnp.sum((states_pred[:, :, 0, :] - observed_positions) ** 2, axis=2)) / 1e6
            vel_err = jnp.mean(jnp.sum((states_pred[:, :, 1, :] - observed_velocities) ** 2, axis=2))
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(PLOT_DIR / "loss_curve.png", bbox_inches="tight")
    plt.close(fig)
    ...
    for alt_km in alt_slices:
        ...
        plot_grid(truth, lon_vals, lat_vals, f"Truth density @ {alt_km:.1f} km", PLOT_DIR / f"density_latlon_truth_{label_alt}.png", vmin, vmax, "Longitude [deg]", "Latitude [deg]")
        plot_grid(learned_hifi, lon_vals, lat_vals, f"Learned density (hi-fi) @ {alt_km:.1f} km", PLOT_DIR / f"density_latlon_learned_hifi_{label_alt}.png", vmin, vmax, "Longitude [deg]", "Latitude [deg]")
        plot_grid(learned_cannonball, lon_vals, lat_vals, f"Learned density (cannonball) @ {alt_km:.1f} km", PLOT_DIR / f"density_latlon_learned_cannonball_{label_alt}.png", vmin, vmax, "Longitude [deg]", "Latitude [deg]")
        ...
        plot_error_grid(err_hifi, lon_vals, lat_vals, f"Percent error (hi-fi) @ {alt_km:.1f} km", PLOT_DIR / f"density_latlon_error_hifi_{label_alt}.png", "Longitude [deg]", "Latitude [deg]", vmax_err)
        plot_error_grid(err_cannonball, lon_vals, lat_vals, f"Percent error (cannonball) @ {alt_km:.1f} km", PLOT_DIR / f"density_latlon_error_cannonball_{label_alt}.png", "Longitude [deg]", "Latitude [deg]", vmax_err)
    ...
    vmin = max(np.nanmin(truth), 1e-16)
    vmax = max(np.nanmax(truth), vmin * 10.0)
    plot_grid(truth, lat_vals_alt, alt_vals, "Truth density @ lon=0 deg", PLOT_DIR / "density_altlat_truth_lon0.png", vmin, vmax, "Latitude [deg]", "Altitude [km]")
    plot_grid(learned_hifi, lat_vals_alt, alt_vals, "Learned density (hi-fi) @ lon=0 deg", PLOT_DIR / "density_altlat_learned_hifi_lon0.png", vmin, vmax, "Latitude [deg]", "Altitude [km]")
    plot_grid(learned_cannonball, lat_vals_alt, alt_vals, "Learned density (cannonball) @ lon=0 deg", PLOT_DIR / "density_altlat_learned_cannonball_lon0.png", vmin, vmax, "Latitude [deg]", "Altitude [km]")
    err_hifi, vmax_hifi = percent_error_grid(learned_hifi, truth)
    err_cbl, vmax_cbl = percent_error_grid(learned_cannonball, truth)
    vmax_err = max(vmax_hifi, vmax_cbl)
    plot_error_grid(err_hifi, lat_vals_alt, alt_vals, "Percent error (hi-fi) @ lon=0 deg", PLOT_DIR / "density_altlat_error_hifi_lon0.png", "Latitude [deg]", "Altitude [km]", vmax_err)
    plot_error_grid(err_cbl, lat_vals_alt, alt_vals, "Percent error (cannonball) @ lon=0 deg", PLOT_DIR / "density_altlat_error_cannonball_lon0.png", "Latitude [deg]", "Altitude [km]", vmax_err)
    ...
    vmin = max(np.nanmin(truth), 1e-16)
    vmax = max(np.nanmax(truth), vmin * 10.0)
    plot_grid(truth, lon_vals_alt, alt_vals, "Truth density @ lat=0 deg", PLOT_DIR / "density_altlon_truth_lat0.png", vmin, vmax, "Longitude [deg]", "Altitude [km]")
    plot_grid(learned_hifi, lon_vals_alt, alt_vals, "Learned density (hi-fi) @ lat=0 deg", PLOT_DIR / "density_altlon_learned_hifi_lat0.png", vmin, vmax, "Longitude [deg]", "Altitude [km]")
    plot_grid(learned_cannonball, lon_vals_alt, alt_vals, "Learned density (cannonball) @ lat=0 deg", PLOT_DIR / "density_altlon_learned_cannonball_lat0.png", vmin, vmax, "Longitude [deg]", "Altitude [km]")
    err_hifi, vmax_hifi = percent_error_grid(learned_hifi, truth)
    err_cbl, vmax_cbl = percent_error_grid(learned_cannonball, truth)
    vmax_err = max(vmax_hifi, vmax_cbl)
    plot_error_grid(err_hifi, lon_vals_alt, alt_vals, "Percent error (hi-fi) @ lat=0 deg", PLOT_DIR / "density_altlon_error_hifi_lat0.png", "Longitude [deg]", "Altitude [km]", vmax_err)
    plot_error_grid(err_cbl, lon_vals_alt, alt_vals, "Percent error (cannonball) @ lat=0 deg", PLOT_DIR / "density_altlon_error_cannonball_lat0.png", "Longitude [deg]", "Altitude [km]", vmax_err)
    ...
    plot_ground_tracks(lon, lat, alts_km, PLOT_DIR / "ground_tracks.png")
